...
ARCHIVE="${1:?Festival patch archive path is required.}"
...
if [[ ! -f "${ARCHIVE}" ]]; then
  echo "Festival patch archive was not found." >&2
  exit 1
fi
...
pg_dump --format=custom "${DATABASE_URL}" \
  --file="${BACKUP_DIR}/database-pre-festival.dump"
psql "${DATABASE_URL}" -v ON_ERROR_STOP=1 \
  -f "${NEW_RELEASE}/database/migrations/20260729_festival_asset_quality_versioning.sql"
