...
source "$ENV"
set +a
pg_dump "$DATABASE_URL" --schema-only --no-owner --no-privileges > "$BACKUP/database-schema-before.sql"
psql "$DATABASE_URL" -v ON_ERROR_STOP=1 -f database/migrations/20260801_admin_feature_notice_state.sql
test "$(psql "$DATABASE_URL" -Atqc "select to_regclass('public.admin_feature_notice_state') is not null")" = t
pnpm exec tsx --test tests/holiday/motif-library.test.ts tests/holiday/festival-studio.test.ts tests/holiday/festival-hero-pilot.test.ts
pnpm build
