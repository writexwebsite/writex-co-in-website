...
test -f "$PATCH_DIR/FestivalPackStudio.tsx"
test -f "$PATCH_DIR/festival-studio.test.ts"
test ! -e "$RELEASE"
...
install -o writexdeploy -g writexdeploy -m 600 \
  "$PATCH_DIR/FestivalPackStudio.tsx" \
  "$RELEASE/components/admin/FestivalPackStudio.tsx"
install -o writexdeploy -g writexdeploy -m 600 \
  "$PATCH_DIR/festival-studio.test.ts" \
  "$RELEASE/tests/holiday/festival-studio.test.ts"
