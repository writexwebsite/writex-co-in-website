...
FILES=(
  ...
  "tests/holiday/login-theme.test.ts"
)
...
runuser -u writexdeploy -- bash -c \
  "cd '$RELEASE' && pnpm exec eslint ${FILES[*]}"
runuser -u writexdeploy -- bash -c \
  "cd '$RELEASE' && pnpm exec tsc --noEmit"
runuser -u writexdeploy -- bash -c \
  "cd '$RELEASE' && pnpm exec tsx --test tests/holiday/login-theme.test.ts tests/holiday/festival-review-batch.test.ts tests/holiday/festival-studio.test.ts"
runuser -u writexdeploy -- bash -c \
