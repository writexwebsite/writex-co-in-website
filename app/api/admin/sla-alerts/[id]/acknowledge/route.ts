...
export async function PATCH(request: NextRequest, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    const result = await dbQuery(
      `
        ...
        set status = 'acknowledged',
            acknowledged_at = now(),
            acknowledged_by_admin_user_id = $2
        where id = $1
        returning id
      `,
    ...
    await logAuditEvent({ actorType: "admin", actorId: session.adminUserId, actorEmail: session.email, entityType: "sla_alert", entityId: id, action: "sla_alert_acknowledged", request });
