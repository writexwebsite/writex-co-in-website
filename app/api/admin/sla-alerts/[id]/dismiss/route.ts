...
export async function PATCH(request: NextRequest, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    const result = await dbQuery(
      "update sla_alerts set status = 'dismissed' where id = $1 returning id",
      [id]
    );
    if (!result.rows[0]) throw new ApiError(404, "NOT_FOUND", "SLA alert was not found.");
    await logAuditEvent({ actorType: "admin", actorId: session.adminUserId, actorEmail: session.email, entityType: "sla_alert", entityId: id, action: "sla_alert_dismissed", request });
