...
import {
  getDevelopmentClientSession,
  revokeDevelopmentClientSession,
  rotateDevelopmentClientSession
} from "@/lib/my-writex/dev-sessions";
...
export async function getClientSessionFromOpaqueToken(token?: string) {
  if (!token) return null;
  const developmentSession = getDevelopmentClientSession(
    token,
...
export async function revokeClientSessionToken(
  ...
  if (!token) return;
  if (revokeDevelopmentClientSession(token)) return;
...
export async function rotateClientSessionToken(token: string | undefined) {
  if (!token) return null;
  const developmentToken = rotateDevelopmentClientSession(token);
...
export function assertInvoiceClientSession(session: ClientSession) {
  if (session.authScope !== "invoice" || !session.invoiceId) {
    throw forbidden("This invoice workspace is not available for this session.");
  }
  return session;
}

export function assertCustomerClientSession(session: ClientSession) {
  if (session.authScope !== "customer" || !session.customerMasterId) {
    throw forbidden("My WriteX is not available for this session.");
  }
  return session;
}

export async function verifyInvoiceClientSessionFromRequest(request: NextRequest) {
  return assertInvoiceClientSession(await verifyClientSessionFromRequest(request));
}

export async function verifyCustomerClientSessionFromRequest(request: NextRequest) {
  return assertCustomerClientSession(await verifyClientSessionFromRequest(request));
}
