...
export function AxoLauncher({ open, prompt, onOpen, onDismissPrompt }: { open: boolean; prompt?: string; onOpen: () => void; onDismissPrompt: () => void }) {
  const [engaged, setEngaged] = useState(false);
  const state: AxoState = open ? "attentive" : prompt ? "welcoming" : engaged ? "curious" : "idle";
  return (
    <div className="flex items-end gap-2">
      ...
      <button type="button" onClick={onOpen} onPointerEnter={() => setEngaged(true)} onPointerLeave={() => setEngaged(false)} onFocus={() => setEngaged(true)} onBlur={() => setEngaged(false)} aria-label={open ? "Return to AXO support" : "Open AXO student support"} aria-expanded={open} className={`group relative min-h-[5.25rem] min-w-[5.25rem] rounded-2xl focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-violet-400/50 sm:min-h-24 sm:min-w-24 ${open ? "" : "wx-axo-roam"}`}>
