...
import { DesignerLoginPackLogo } from "@/components/auth/DesignerLoginThemeRenderer";
...
export function AxoLoginTransition({ children }: { children: ReactNode }) {
  ...
  return (
    <div className={`wx-auth-card relative flex min-h-0 w-full min-w-0 max-w-none flex-col justify-center rounded-[1.75rem] border p-6 transition duration-300 sm:p-8 lg:p-8 ${active ? "border-wxViolet700 shadow-soft" : "border-wxBorder shadow-soft"}`}>
      <div className="wx-auth-card-logo mb-6 flex justify-center sm:mb-8">
        <DesignerLoginPackLogo
          className="wx-auth-card-logo-mark h-auto w-60 object-contain sm:w-72"
          fallback={
            <BrandLogo
