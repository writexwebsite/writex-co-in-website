...
type HiringPerson={id:string;name:string;email:string;hiringRole:string};
type HiringInterview={id:string;interview_type:string;status:string};
...
export function HiringOperationsConsole({view,applicationReference,verificationType,hiringPeople=[],interviews=[]}:{view:View;applicationReference?:string;verificationType?:string;hiringPeople?:HiringPerson[];interviews?:HiringInterview[]}){
...
function fieldsFor(view:View,verificationType:string|undefined,hiringPeople:HiringPerson[],interviews:HiringInterview[]):ReactNode{
  ...
  if(view==="interviews")return <><Select name="action" label="Operation" options={["schedule","reschedule","complete","cancel","no_show"]}/><SelectPairs name="interviewId" label="Current interview (for update/complete)" options={interviews.map(interview=>({value:interview.id,label:`${humanise(interview.interview_type)} - ${humanise(interview.status)}`}))} optional/><Select name="interviewType" label="Interview type" options={["screening","role_interview","viva","management","final"]} optional/><SelectPairs name="interviewerAdminUserId" label="Interviewer" options={hiringPeople.filter(person=>["super_admin","hr_admin","hiring_manager","interviewer"].includes(person.hiringRole)).map(person=>({value:person.id,label:`${person.name} - ${humanise(person.hiringRole)}`}))} optional/><Field name="scheduledAt" label="Schedule" type="datetime-local"/><Field name="durationMinutes" label="Duration minutes" type="number" defaultValue="30"/><Select name="recommendation" label="Recommendation" options={["advance","hold","talent_pool","not_selected"]} optional/><fieldset className="grid gap-4 rounded-md border border-wxBorder p-4 md:col-span-2 sm:grid-cols-2"><legend className="px-1 text-sm font-bold text-wxIndigo900">Structured interview scorecard</legend><Field name="scoreCommunication" label="Communication score" type="number"/><Field name="scoreRoleKnowledge" label="Role knowledge score" type="number"/><Field name="scoreProblemSolving" label="Problem-solving score" type="number"/><Field name="scoreIntegrity" label="Integrity and judgement score" type="number"/></fieldset><Field name="notes" label="Structured notes" textarea wide/></>;
...
function Field({name,label,placeholder,type="text",required=false,textarea=false,wide=false,defaultValue,readOnly=false}:{name:string;label:string;placeholder?:string;type?:string;required?:boolean;textarea?:boolean;wide?:boolean;defaultValue?:string;readOnly?:boolean}){const className="mt-2 min-h-11 w-full rounded-md border border-wxBorder bg-wxSurface px-3 py-2 text-sm outline-none focus:border-wxViolet700 focus:ring-2 focus:ring-wxViolet700/20";return <label className={`text-sm font-semibold text-wxIndigo900 ${wide?"md:col-span-2":""}`}>{label}{textarea?<textarea name={name} required={required} placeholder={placeholder} rows={4} className={className}/>:<input name={name} required={required} readOnly={readOnly} placeholder={placeholder} type={type} defaultValue={defaultValue} className={className}/>}</label>}
...
function SelectPairs({name,label,options,optional=false}:{name:string;label:string;options:Array<{value:string;label:string}>;optional?:boolean}){return <label className="text-sm font-semibold text-wxIndigo900">{label}<select name={name} required={!optional} defaultValue="" className="mt-2 min-h-11 w-full rounded-md border border-wxBorder bg-wxSurface px-3 py-2 text-sm"><option value="">{options.length?"Select":"No eligible records available"}</option>{options.map(option=><option key={option.value} value={option.value}>{option.label}</option>)}</select></label>}
